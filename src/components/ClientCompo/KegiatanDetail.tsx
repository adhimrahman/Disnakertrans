...
export default function KegiatanDetail({ kegiatan, semuaKegiatan }: Props) {
	const { id } = useParams();

	const index = useMemo(() => semuaKegiatan.findIndex((item) => item.id === id), [id, semuaKegiatan]);
	const kegiatanSebelumnya = semuaKegiatan[index - 1];
	const kegiatanBerikutnya = semuaKegiatan[index + 1];
			...
			<main className="container mx-auto max-w-6xl px-4 pt-2 pb-10 grid grid-cols-1 md:grid-cols-3 gap-8">
				<div className="md:col-span-2">
					...
					<article className="prose prose-lg prose-slate text-gray-900 max-w-none text-justify mb-10">
						<p className="whitespace-pre-line">{kegiatan.Deskripsi}</p>
					</article>
