...
import { deletePelatihan } from "@/firebase/utils/pelatihan-service";
...
export default function PelatihanList({ pelatihan, lpkId, pageSize = 10 }: PelatihanListProps) { 
      ...
      <table className="min-w-full table-auto shadow-md">
        <thead className="bg-gray-100 text-sm font-medium">
          <tr className="text-white bg-steelBlue">
            ...
            <th className="px-4 py-3 min-w-[180px] text-center">Judul</th>
            <th className="px-4 py-3 min-w-[140px] text-center">Tanggal Kegiatan</th>
            <th className="px-4 py-3 text-center">Link Form</th>
            <th className="px-4 py-3 min-w-[120px] text-center">Link Konten</th>
            <th className="px-4 py-3 text-center">Aksi</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-500">
          {visibleRow.length > 0 && pelatihan.length > 0 ? (
            visibleRow.map((item, index) => (
              <tr key={item.id} className="text-sm text-gray-800 font-medium hover:bg-blue-50 transition-colors duration-150 ease-in-out">
                ...
                <td className="px-4 py-3">{item.judul || "Tidak ada"}</td>
                <td className="px-4 py-3 text-center">{item.tanggal_kegiatan|| "Tidak ada"}</td>
                ...
                <td className="px-4 py-3">
                  <div className="flex space-x-2 justify-center">
                    <button
                      ...
                      onClick={() => {redirect(`/dashboard/lpk/${lpkId}/pelatihan/edit/${item.id}`)}}
                    >
      ...
      <div className="flex flex-row justify-between">
        <button
          className="bg-steelBlue hover:bg-darkBlue transition px-4 py-2 rounded-xl text-white w-full lg:w-auto hover:cursor-pointer"
            onClick={() => redirect(`/dashboard/lpk/${lpkId}/pelatihan/add`)}
        >
          + Tambah Konten
        </button>
        <PaginationControls
