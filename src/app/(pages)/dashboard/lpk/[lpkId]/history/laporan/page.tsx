...
// Removed import of Set from "immutable"
...
export default function LaporanLpkPage() {
    ...
    const [selectedYear, setSelectedYear] = useState<string>("Semua"); // ✅ TAMBAHAN
  ...
  const getAvailableYears = () => {
    const years = new Set<string>(); // Using native JavaScript Set
    laporan.forEach((item) => {
      const year = item.waktu_pelatihan.toDate().getFullYear().toString();
      years.add(year);
    });
    return Array.from(years).sort((a, b) => Number(b) - Number(a)); // Sort years in descending order
  };

  // Removed redundant sorted declaration as filteredData already handles sorting
  ...
  const filteredData = useMemo(() => {
    ...
    const filtered = laporan.filter((item) => {
      ...
      const yearMatch = selectedYear === "Semua" || item.waktu_pelatihan.toDate().getFullYear().toString() === selectedYear;
