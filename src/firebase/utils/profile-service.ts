...
export async function updateProfileData(formData: Partial<Profile>, files: { gambar?: File }) {
	...
	if (!formData.id) { throw new Error('Document ID is required') }

	const docRef = doc(db, "profile", formData.id);
	const data = { ...formData, gambar: gambarUrl };
...
export async function getDefaultProfile(): Promise<updateProfile | null> {
	const id = process.env.NEXT_PUBLIC_PROFILE_ID;
	if (!id) {
		throw new Error("NEXT_PUBLIC_PROFILE_ID tidak ditemukan di .env");
	}
