...
import {
  ...
  limit, orderBy, query, QueryDocumentSnapshot,
  startAfter, Timestamp, updateDoc, where
...
export async function getAduan(): Promise<Partial<AduanItem[]>> {
  const collectionRef = collection(db, "aduan");
  const q = query(collectionRef, where("is_done", "==", false));
  const aduanCollectionSnapshot = await getDocs(q);
...
// export async function deleteAduanById(id: string) {
//   const validateId = Validation.validate(getAduanSchema, id);
//   const docRef = doc(db, "aduan", validateId);
...
export async function getAduanBySort(
  ...
  let q = query(collectionRef, where("is_done", "==", false));
